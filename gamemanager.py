...
class GameManager:
    def __init__(self):
        ...
        self.news = [
            (0, "Местные власти сообщают об ухудшении погодных условий."),
            (0, "В новостях объявили о возможных перебоях с электричеством из-за ближайших боев."),
            (0, "Соседний город подвергся массированной атаке. Жители в панике."),
            (0, "Гуманитарная помощь скоро должна прибыть в ближайшие дни."),
            (0, "Вчера вечером был сбит вражеский беспилотник над городом."),
            (0, "Сообщается о крупном пожаре на одном из промышленных предприятий."),
            (0, "Ополченцы заявили о контрнаступлении в ответ на обстрелы."),
            (5, "Мирные переговоры начнутся в следующую неделю, сообщают источники."),
            (5, "Жители города организуют общественную оборону."),
            (5, "Начались массовые эвакуации из близлежащих населённых пунктов."),
            (10, "Обнаружены новые очаги заражения воды в результате бомбежек."),
            (10, "Международные организации обещают увеличение объёмов помощи."),
            (10, "В районе конфликта увеличивается число беженцев."),
            (10, "Заявления о нарушениях перемирия участились в последние дни."),
            (15, "Спутниковые снимки показывают движение войск противника к границе."),
            (15, "Новая волна беженцев движется в сторону вашего региона."),
            (15, "Местные жители сообщают о замеченных минных полях на прилегающих территориях."),
            (15, "Власти призывают граждан не покидать убежища без крайней необходимости."),
            (20, "Появились сообщения о заключении перемирия."),
            (20, "Местные власти начинают расследование военных преступлений."),
            (20, "Отмечается увеличение числа жертв среди мирного населения."),
            (20, "Ожидается прибытие международных наблюдателей для мониторинга ситуации."),
            (20, "СМИ сообщают о приостановке военных действий для обеспечения доставки гуманитарной помощи.")
        ]

    def broadcast_news(self):
        # Выбор новостей, которые разрешены для текущего дня
        available_news = [news for min_day, news in self.news if min_day <= self.current_day]
        if available_news:
            news_of_the_day = random.choice(available_news)
            print(f"Новости дня: {news_of_the_day}")
        else:
            print("Сегодня без новостей.")
    ...
    def check_endings(self):
        ...
        if alive_count == total_count:
            ...
            print(
                "Никто не выжил. Ваша история заканчивается здесь, в трагедии и разрушении.")
            good_end = False
        if good_end:
